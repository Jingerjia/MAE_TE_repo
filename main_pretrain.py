...
def get_args_parser():
    ...
    parser.add_argument("--dataset", type=str, default="audioset", help="dataset", choices=["audioset", "esc50", "speechcommands", 'taiwan_birds'])
...
def main(args):
    ...
    if not args.audio_exp:
        ...
        norm_stats = {'audioset':[-4.2677393, 4.5689974], 'esc50':[-6.6268077, 5.358466], 'speechcommands':[-6.845978, 5.5654526], 'taiwan_birds':[-2.6699, 1.9479]}
        target_length = {'audioset':1024, 'esc50':512, 'speechcommands':128, 'taiwan_birds':128}
        multilabel_dataset = {'audioset': True, 'esc50': False, 'k400': False, 'speechcommands': True, 'taiwan_birds':True}
    ...
    torch.cuda.nvtx.range_push('model_loading')########################
    # define the model
    if args.audio_exp:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        torch.cuda.nvtx.range_push(f'epoch_{epoch}_training')#
        ...
        torch.cuda.nvtx.range_push(f'epoch_{epoch}_recording')########
        #print('\n\nepoch = ', epoch)
        #print('args.save_every_epoch = ', args.save_every_epoch)
        #print("epoch", "%", "args.save_every_epoch = ",epoch % args.save_every_epoch)
        #print('epochs = ', args.epochs,'\n\n')
        #print('args.output_dir = ', args.output_dir)
        if args.output_dir and (epoch % args.save_every_epoch == 0 or epoch + 1 == args.epochs):
            torch.cuda.nvtx.range_push(f'save_model')#
            #print("start saving pth file\n\n")
            misc.save_model(
        ...
        if args.output_dir and misc.is_main_process():
            torch.cuda.nvtx.range_push(f'log_write')#
