...
from timm.layers import PatchEmbed, DropPath, PatchDropout, \
    resample_patch_embed, resample_abs_pos_embed, \
    get_act_layer, get_norm_layer, LayerType

from timm_dep.layers.mlp import Mlp, SwiGLUPacked
from timm_dep.layers.attention_pool import AttentionPoolLatent
from timm_dep.layers.norm import RmsNorm
from timm_dep.layers.weight_init import trunc_normal_, lecun_normal_
from timm_dep.layers.config import use_fused_attn

from timm_dep._builder import build_model_with_cfg
from timm_dep._features import feature_take_indices
from timm_dep._manipulate import named_apply, checkpoint_seq, adapt_input_conv
from timm_dep._registry import generate_default_cfgs, register_model, register_model_deprecations
...
class Attention(nn.Module):
    ...
    def __init__(
            ...
            attn_drop: float = 0.,
            proj_drop: float = 0.,
            norm_layer: nn.Module = te.LayerNorm,
    ) -> None:
        super().__init__()  
...
class ParallelScalingBlock(nn.Module):
    ...
    def __init__(
        ...
        self.in_proj = te.Linear(dim, in_proj_out_dim, bias=qkv_bias)
        ...
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()
        self.attn_drop = nn.Dropout(attn_drop)
        self.attn_out_proj = te.Linear(dim, dim)

        self.mlp_drop = nn.Dropout(proj_drop)
        self.mlp_act = act_layer()
        self.mlp_out_proj = te.Linear(mlp_hidden_dim, dim)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.head = te.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = '') -> None:
    ...
    if (isinstance(model.head, te.Linear) and
            f'{prefix}head/bias' in w and
            model.head.bias.shape[0] == w[f'{prefix}head/bias'].shape[-1]):
        model.head.weight.copy_(_n2p(w[f'{prefix}head/kernel']))
        model.head.bias.copy_(_n2p(w[f'{prefix}head/bias']))
    # NOTE representation layer has been removed, not used in latest 21k/1k pretrained weights
    # if isinstance(getattr(model.pre_logits, 'fc', None), te.Linear) and f'{prefix}pre_logits/bias' in w:
...
def checkpoint_filter_fn(
    ...
    if 'visual.class_embedding' in state_dict:
        ...
        prefix = 'visual.trunk.'
        if 'visual.head.proj.weight' in state_dict and isinstance(model.head, te.Linear):
            # remap final te.Linear if it exists outside of the timm .trunk (ie in visual.head.proj)
