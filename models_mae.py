...
import quickstart_utils as qutils
import transformer_engine.pytorch as te


#from timm.models.vision_transformer import PatchEmbed, Block
from vit_te import Block
...
from swin_te_orgi import SwinTransformerBlock
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=10, in_chans=3,
                 ...
                 mlp_ratio=4., norm_layer=te.LayerNorm, norm_pix_loss=False, 
                 ...
                 mask_t_prob=0.6, mask_f_prob=0.5, mask_2d=False, target_length=8,
        ...
        self.decoder_embed = te.Linear(embed_dim, decoder_embed_dim, bias=True)
        ...
        self.decoder_pred = te.Linear(decoder_embed_dim, patch_size**2 * in_chans, bias=True) # decoder to patch
        ...
        self.epoch = epoch
        self.target_length = img_size[0]
        print(f'self.target_length = {self.target_length}')
    ...
    def random_masking_2d(self, x, mask_t_prob, mask_f_prob):
        ...
        if self.use_custom_patch: # overlapped patch
            ...
            T=int(self.target_length/16)
            F=8
        #x = x.reshape(N, T, F, D)
        len_keep_t = int(T * (1 - mask_t_prob)) # time domain 剩餘 patch 數
        len_keep_f = int(F * (1 - mask_f_prob)) # frequency domain 剩餘 patch 數
    ...
    def forward(self, imgs, mask_ratio=0.8):
        ...
        torch.cuda.nvtx.range_push('patchify')
        target = self.patchify(imgs)
        torch.cuda.nvtx.range_pop()
        return loss_recon, pred, mask, loss_contrastive, imgs, target
